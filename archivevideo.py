...
def quote_args(args):
    ...
    """
    return " ".join(map(lambda a: quote(a), args))
...
def ffmpeg_args(meta, tgt):
    ...
    if options.x265:
        ...
        if len(x265params) > 0:
            args.extend(["-x265-params", ",".join(x265params)])
    else:
...
def main(argv=None):
    ...
    cmd = transcode
    if argv is None:
        #argv = list(map(lambda s: s.decode('UTF-8'), sys.argv))
        argv = sys.argv
    ...
    try:
        ...
        print("Usage: archivevideo [-{}] file...".format(flags), file=sys.stderr)
